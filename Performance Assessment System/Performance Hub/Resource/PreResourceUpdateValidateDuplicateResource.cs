...
{
    /// <summary>
    /// PreValidation-Operation Plugin on Update of Resource.
    /// Prevents duplicate Resource records based on First Name + Last Name + Reporting Manager + Designation.
    /// Filter Attribute: ink_firstname, ink_lastname, ink_reportingmanager, ink_designation
    /// Pre-Image Alias: PreValidationResourceUpdateValidateDuplicateResourcePreImage
    /// </summary>
        ...
        public void Execute(IServiceProvider iServiceProvider)
        {
            ...
            try
            {
                if (Plugin.ValidateTargetAsEntity(CommonEntities.RESOURCE, iPluginExecutionContext))
                {
                    ...
                    {
                        Entity resourcePreImageEntity = Plugin.GetPreEntityImage(iPluginExecutionContext, preImageAlias);

                        if (resourcePreImageEntity != null)
                        {
                            string firstName = Plugin.GetAttributeValue<string>(
                                resourceEntity,
                                resourcePreImageEntity,
                            ...
                            resourceQueryExpression.Criteria.AddCondition(
                                CommonEntities.Resource.INK_RESOURCEID,
